...
class TestReadFlac < MiniTest::Test
    ...
    def setup
        ...
        @flac.album = "album"
        @mp3 = GenericTag::Metadata.convert(:id3v24,@flac)
    end
    ...
    def test_basic_reading
        ...
        assert_equal ["album"], @flac.album
    end

    def test_conversion
        assert_equal ['alb1_track_1'], @mp3.musicbrainz_recordingid 
        assert_equal ['alb1_track_1'], @mp3.tags[:"UFID://musicbrainz.org"].values
        assert_equal [], @mp3.artist
        assert_nil @mp3.tags[:TPER]
        assert_equal ["album"], @mp3.album
        assert_equal ["album"], @mp3.tags[:TALB].values
    end
